...
class HealthCheckModule:
    ...
    def run(self, devices: list[DeviceInfo], serial: str | None = None) -> dict:
        ...
        if adb_path:
            checks.append(
                self._check("adb_binary", "OK", f"ADB trouve: {adb_path}", "")
            )
        else:
            checks.append(
                self._check(
        ...
        if version.ok:
            ...
            checks.append(
                self._check(
                    "adb_version",
                    "ERROR",
                    version.stderr or "Impossible de lire la version ADB",
                    "Verifier l'installation ADB.",
                )
            )
        ...
        if server.ok:
            checks.append(
                self._check(
                    "adb_server", "OK", server.stderr or "Serveur ADB actif", ""
                )
            )
        else:
            checks.append(
        ...
        if online:
            checks.append(
                self._check(
                    "device_detected", "OK", f"{len(online)} appareil(s) detecte(s)", ""
                )
            )
        else:
            checks.append(
        ...
        if online:
            ...
            checks.append(
                self._check(
                    "transport",
                    "OK",
                    f"Transport(s): {', '.join(sorted(transports))}",
                    "",
                )
            )
        else:
        ...
        if target:
            ...
            checks.append(
                self._check(
                    "latency",
                    "WARNING",
                    "Latence non testee (aucun appareil actif)",
    ...
    def _latency_check(self, serial: str) -> dict[str, str]:
        ...
        if res.ok and "ping" in res.stdout:
            ...
            remediation = (
                ""
                if status == "OK"
                else "Connexion lente: preferer USB ou rapprocher le Wi-Fi."
            )
            return self._check(
                "latency",
                status,
                f"Latence commande simple: {dt_ms:.1f} ms",
                remediation,
            )
        return self._check(
            "latency",
    ...
    def _critical_non_destructive_checks(self, serial: str) -> list[dict[str, str]]:
        ...
        checks = [
            (
                ...
                "getprop inaccessible",
            ),
            (
                "cmd_pm",
                ["shell", "pm", "list", "packages", "-3"],
                "pm list packages echoue",
            ),
        ]
        for name, cmd, label in checks:
            ...
            if res.ok:
                out.append(
                    self._check(name, "OK", "Commande critique non destructive OK", "")
                )
            else:
                out.append(
                    self._check(
    ...
    def _check(
        self, name: str, status: str, message: str, remediation: str
    ) -> dict[str, str]:
        return {
            "name": name,
            "status": status,
