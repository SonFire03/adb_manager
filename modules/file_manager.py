...
class FileManagerModule:
    ...
    def pull(self, serial: str, remote_path: str, local_path: Path) -> CommandResult:
        ...
        return self.adb.run(
            ["pull", remote_path, str(local_path)], serial=serial, timeout=180
        )

    def push(self, serial: str, local_path: Path, remote_path: str) -> CommandResult:
        return self.adb.run(
            ["push", str(local_path), remote_path], serial=serial, timeout=180
        )
    ...
    def search_remote(self, serial: str, base_path: str, pattern: str) -> list[str]:
        result = self.adb.run(
            ["shell", "find", base_path, "-type", "f"], serial=serial, timeout=45
        )
        if not result.ok:
            return []
        return [
